...
public final class XmlSparqlToUser {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(XmlSparqlToUser.class);

    private XmlSparqlToUser() {
        // Not Called
    }

    public static User createUser(final String xmlSparqlResult, final String uri) {
        User user = new User();
        ...
        try {
            ...
            for (int j = 0; j < bindingList.getLength(); j++) {
                ...
                if ("hasEmail".equals(name)) {
                    user.setMboxFoaf(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
                if ("hasName".equals(name)) {
                    user.setNameFoaf(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
            }
