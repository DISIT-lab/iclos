...
public final class XmlSparqlToBusinessConfiguration {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(XmlSparqlToBusinessConfiguration.class);

    private XmlSparqlToBusinessConfiguration() {
        // Not Called
    }

    public static BusinessConfiguration createBusinessConfiguration(
        ...
        try {
            ...
            for (int j = 0; j < bindingList.getLength(); j++) {
                ...
                if ("hasIdentifier".equals(name)) {
                    businessConfiguration.setHasIdentifier(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
                if ("hasName".equals(name)) {
                    businessConfiguration.setHasName(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
                if ("hasContractId".equals(name)) {
                    businessConfiguration.setHasContractId(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
            }

            businessConfiguration.setUri(uri);
