...
public final class XmlSparqlToIcaroApplication {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(XmlSparqlToIcaroApplication.class);

    private XmlSparqlToIcaroApplication() {
        // Not Called
    }

    public static IcaroApplication createIcaroApplication(
            final String xmlSparqlResult, final String uri) {
        IcaroApplication icaroApplication = new IcaroApplication();
        ...
        try {
            ...
            for (int j = 0; j < bindingList.getLength(); j++) {
                ...
                if ("hasIdentifier".equals(name)) {
                    icaroApplication.setHasIdentifier(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
                if ("hasName".equals(name)) {
                    icaroApplication.setHasName(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
                if ("hasCapacity".equals(name)) {
                    icaroApplication.setHasCapacity(Float
