...
public final class XmlSparqlToLocalStorage {

    private static final Logger LOGGER = LoggerFactory
            .getLogger(XmlSparqlToLocalStorage.class);

    private XmlSparqlToLocalStorage() {
        // Not Called
    }

    public static LocalStorage createLocalStorage(final String xmlSparqlResult,
            final String uri) {
        ...
        try {
            ...
            for (int j = 0; j < bindingList.getLength(); j++) {
                ...
                if ("hasName".equals(name)) {
                    localStorage.setHasName(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
                if ("hasIdentifier".equals(name)) {
                    localStorage.setHasIdentifier(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent());
                }
                if ("hasDiskSize".equals(name)) {
                    localStorage.setHasDiskSize(Float.parseFloat(bindingElement
                            .getElementsByTagName(XmlSparqlUtility.LITERAL)
                            .item(0).getTextContent()));
                }
            }

            localStorage.setUri(uri);
