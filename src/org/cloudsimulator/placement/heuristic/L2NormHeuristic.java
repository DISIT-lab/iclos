...
public class L2NormHeuristic extends HeuristicControllerBase{
    ...
    protected ArrayList<ScoredVM> computeScore(ArrayList<VirtualMachinePlacement> vmFittingBin,HostMachinePlacement openedBin,
        ...
        for (int i = 0; i < score.length; i++) {           
            DoubleMatrix temp = weigthed_requested.getRow(i).sub(openedBin.getResidual_resource());
            temp.muli(temp);
            score[i] = (float) this.getMeanResourceDemandCoefVec().dot(temp);
