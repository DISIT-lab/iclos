...
public abstract class HeuristicControllerBase implements HeuristicGeometric{

    protected DoubleMatrix meanResourceDemandCoefVec;
    protected int max_score_item_index;
    
    public VirtualMachinePlacement chooseBestVm(ArrayList<VirtualMachinePlacement> vmFittingBin, HostMachinePlacement openedBin) {
                
        DoubleMatrix weigthed_requested = VirtualMachinePlacement.buildRequestedResourceMatrix(vmFittingBin);        
        
        ArrayList<ScoredVM> scoredVMs = computeScore(vmFittingBin,openedBin, weigthed_requested);        
        
        return selectVM(vmFittingBin,scoredVMs);
    }
    
    abstract ArrayList<ScoredVM> computeScore(
            ArrayList<VirtualMachinePlacement> vmFittingBin,
            HostMachinePlacement openedBin, DoubleMatrix weigthed_requested);
    ...
    protected VirtualMachinePlacement selectVM(ArrayList<VirtualMachinePlacement> vmFittingBin, ArrayList<ScoredVM> scoredVMs) {
        return vmFittingBin.get(max_score_item_index);
    }
